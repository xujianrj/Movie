...
class TagAction extends BaseAction{	
    ...
    public function showajax(){
		$rs = D("Tag");
		$array = $rs->field('*,count(tag_name) as tag_count')->limit('100')->group('tag_name,tag_sid')->order('tag_count desc')->select();
		foreach($array as $key=>$val){
			if($array[$key]['tag_sid']==2){
				$array[$key]['tag_url']=UU('Admin-News/Show',array('tag_name'=>urlencode($array[$key]['tag_name']),'tag_type'=>1),'',false,true);
			}else{
